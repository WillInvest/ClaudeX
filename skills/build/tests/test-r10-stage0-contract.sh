...
THINK_START="$ROOT/skills/think/scripts/start-tmux-build.sh"
...
grep -q '\[test-only/manual\] running outside tmux; continuing' "$SKILL"
...
grep -q 'start-tmux-build.sh' "$THINK_START"
grep -q 'tmux new-session -d' "$THINK_START"
grep -q 'STABLE_PROMPT="$RUN_DIR/.tmux-prompt.md"' "$THINK_START"
grep -q -- '-e "CXMEM_HOME=' "$THINK_START"
! grep -q '\[test-only/manual\]' "$THINK_START"

echo "PASS: build Stage 0 documents resolver routing, env precedence, CXMem states, and manual tmux warning"
